...
4. 控制符号导出：
    * **`static` 关键字：**对于 C/C++ 程序而言，具有文件作用域的标识符，默认具有外部链接，也就是说链接器在处理这些标识符的时候，会将这些标识符的每一个声明指向同一个对象或函数。通过使用 `static` 关键字修饰标识符，可以将其链接属性改为内部链接。那么链接器将得不到该标识符的相关信息。但是这种方法使得在源代码中，符号也无法共享。
    * **定义可见性属性（GNU）：** ELF application binary interface(ABI) defines the visibility of symbols.Generally,it defines four classes,but in most cases, only two of them are more commonly used:
        * STV_DEFAULT：该属性定义的符号会进行导出。
        * STV_HIDDEN：该属性定义的符号不会进行导出，也就不能被其他对象使用。
        * STV_PROTECTED：该属性定义的符号可以导出，但是不能进行覆盖。也就是说，持有该符号的库在库内部始终使用自己的版本，即使外部定义了同名符号。
        * STV_INTERNAL：该属性定义的符号不会进行导出，仅仅在当前可执行文件或共享库内部可见，外部无法访问。
    
        这种方法可以让符号在源代码中进行使用，只是生成库的时候不会导出给其他对象使用。In fact，the hidden symbols would not appear in the dynamic symbol table, but is left in the symbol table for static lingking purpose.
    
    * **使用 `export list`**：通过链接器来控制符号导出。编写一个 export map 文件，然后使用 gcc 编译的时候通过 --version-script 选项进行传递。
