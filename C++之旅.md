...
1. 为了避免使用非限定模板时造成的歧义，标准规定了范围版本必须在作用域内显示声明：
   ```cpp
   sort(v); //错误，std中没有匹配的函数
   ranges::sort(v); //正确
   using ranges::sort;
   sort(v); 
   ```
...
1. C风格字符串：一个以 `0`(ASCII 码)结尾的 `char` 数组。

2. 一个字符串字面量类型是 `const char *` 类型，要想获得 `std::string` 类型的字面量可以加上 `s` 后缀，但是必须使用特定的命名空间。
   ```cpp
   using std::literals::string_literals;
   auto cat = "Cat"s; //std::string 类型
   auto dog = "Dog"; //const char * 类型
   string s;
   s.c_str(); //将string类型转化为c风格字符串进行只读访问的接口
   ```

3. `string_view` ：字符串视图类型，允许访问基于字符的连续序列，字符可以使用 `string` 以及 C 风格字符串进行存储。但是它是**只读**的。

   ```cpp
   string king = "Hello, world";
   string_view s1{&king[0], 2}; //s1:He，第一个参数是位置，第二个参数是长度
   ```

4. 原始字符串字面量：以 `R"(` 开始，以 `)` 结束，可以直接包含反斜线和引号而无需转义。



## 第十一章 输入和输出

1. `ostream` 类型将有类型的对象转换为字符（字节）流，`istream` 将字符（字节）流转换为有类型的对象。这些操作都是类型安全且类型敏感的，都能扩展用以处理用户自定义类型。

2. 输出表达式的结果，即 `<<` 的结果是**输出流的引用**，所以可以用来继续进行输出。

3. 输入表达式中，默认情况下，空白符（如空格或者换行）会终止输入。可以使用 `getline()` 函数来读取一整行，包括结束的换行符。

4. 可以使用 **格式控制符** 来进行格式化控制，使用 `precision()` 对浮点值进行显示控制时不会进行简单的截断，而是会进行四舍五入。

5. 在 `format` 中，标准库提供了类型安全但不可扩展的 `printf()` 风格的格式化机制，这个机制的核心函数是 `format()` 函数，其返回 `string` 类型。
...
11. 同步流：在多线程系统中，I/O变得不可靠，除非：

    * 只有一个 `thread` 在使用流
    * 访问流的操作进行了同步，以确保同一时刻只有一个 `thread` 获得访问权。

12. 如果不适用 C 风格 I/O 并且在意 I/O 性能，可以调用：
    ```cpp
    ios_base::sync_with_stdio(false); //避免显著开销
    ```

13.  `<filesystem>` 提供了很多处理文件操作的接口。

## 第十二章 容器

1. `vector` 是一个给定类型元素的序列，元素在内存中是连续存储的。典型的 `vector` 实现会包含一个句柄，保存指向首元素的指针，还会包含指向尾元素之后位置的指针以及指向所分配空间之后位置的指针。

2. vector 的设计思想：
   ```cpp
   template <typename T>
   class Vector{
     	allocator<T> alloc; //T类型元素的标准库分配器
     	T* elem;            //指向首元素指针
       T* space;		    //指向第一个未使用（且未初始化）位置的指针
       T* last;            //指向最后一个存储位置的指针
   public:
       int size() const{
           return space - elem; //元素数量
       }
       int capacity() const{
           return last - elem; //元素可用空间
       }
       void reserve(int newSize); //将capacity()增长到新尺寸
   };
   //push_back并不会将capacity简单+1，而是会扩展到现有的二倍，从而避免多次迁移元素。
   #include <vector>
   #include <iostream>
   
   int main(){
       std::vector<int> v_a(10, 0);
       v_a.push_back(10);
       std::cout << "Vector size is " << v_a.size() << std::endl;
       std::cout << "Vector capacity is " << v_a.capacity() << std::endl;
       return 0;
   }
   ```
...
5. `map` 是一棵平衡二分搜索树（红黑树），搜索的时间复杂度为O(log(n))，它的下标操作本质上是一次搜索，如果没有找到 `key` ，则向 `map` 中插入一个新元素，该元素的键为 `key` ，值为 `value` 类型的默认值。

6. `unordered_map` 是使用哈希来存储的。标准库为 `string` 及其他内置类型和标准库类型提供了默认的哈希函数。

## 第十三章 算法

1. 标准库算法都被描述为元素序列（半开半闭区间）上的操作，序列（sequence）由一对迭代器表示，分别指向首元素和尾后位置。
2. 迭代器的重要作用是分离算法和容器（数据结构），算法通过迭代器来处理数据，但是它对存储元素的容器一无所知，而容器只用来存储元素，对于处理其元素使用的算法也是一无所知。
3. 任何一种特定的迭代器都是某种类型的对象，每个迭代器都是与某个特定的容器类型相关联，所以需要保存一些必要信息，以便对容器完成某些任务。
4. * 并行执行：任务在多个线程中完成，往往在不同的处理器核心中运行。
   * 数组化执行（又称向量化执行）：任务在单线程中使用数组处理器（又称向量处理器）执行，SIMD（单指令多数据）。

## 第十四章 范围

1. 视图是查看范围的一种方式，当从视图类型读取数据的时候，从它对应的范围开始，如果值满足特定谓词则返回对应元素，否则在范围内取下一个元素。视图又被称为范围适配器。

2. 对于每个标准库视图，标准库都提供了可以产生过滤器的函数，也就是可以作为过滤器操作符 `|` 的参数的对象。
...
1. 指针更泛化的概念是任何允许我们引用对象并根据其类型访问它的东西。
2. **拥有所有权** 的指针意味着它负责删除所指向的对象，
3. * `unique_ptr`：代表唯一所有权，它的析构函数会销毁它的对象。
   * `shared_ptr` ：代表共享所有权，最后一个共享的析构函数负责销毁对象。
...
1. 将函数作为一个函数的参数传递时，参数的类型必须与被调用函数声明中表达的期望完全匹配，如果只是大致符合预期而没有完全匹配，则可以使用以下方法：
   * 使用匿名函数
   * 使用 `std::mem_fn()` 将成员函数转化为函数对象
   * 让函数接受 `std::function` 作为参数

## 第十八章 并发

1. C++提供了合适的内存模型和一套原子操作：
   * 原子操作允许进行无锁编程
   * 内存模型确保只要程序员避免数据竞争，一切就会像期望的那样工作。
2. C++支持协程，也就是说函数可以在调用之间保持它们的状态。
3. 程序的所有线程共享单一地址空间。
4. 标准库提供了一些特性，允许在抽象的任务层（工作并发执行）进行操作，而不是在底层的线程和锁的层次直接进行操作：
   *  `future` 和 `promise` 用来从一个独立线程上创建出的任务返回值。
     当一个任务需要向另一个任务传输值时，它将值放入 `promise` 中，然后 C++ 实现以某种方式令这个值出现在对应的 `future` 中，另一个任务就可以进行读取。
   * `package_task` 是帮助启动任务及连接返回结果的机制。
